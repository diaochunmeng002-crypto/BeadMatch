...
写死就这 6 个（跟串珠、推理同一套实物）：``Y`` 黄、``G`` 绿、``R`` 红、``P`` 紫、``B`` 蓝、``O`` 橙。

卡里三种写法都认 —— ``R`` / ``红`` / ``🔴``；连写也行（``RRY``、"红黄红红"、"🔴🟡🔴🔴"）。
...
def to_letters(text) -> Tuple[str, ...]:
    ...
    分隔用空格/逗号都行（``"R R R Y"``），也可以连写（``"RRRY"``、``"红黄红红"``、
    ``"🔴🟡🔴🔴"``）。列表也直接收。
    """
...
def counts_of(letters: Sequence[str]) -> List[Dict[str, object]]:
    ...
    ``("R","R","R","Y")`` → ``[{color: "R", name: "红色", emoji: "🔴", count: 3},
    {color: "Y", name: "黄色", emoji: "🟡", count: 1}]``

    排序：**多的排前面**（看着像"主角"），一样多就按固定颜色顺序。
    """
    tally: Dict[str, int] = {}
