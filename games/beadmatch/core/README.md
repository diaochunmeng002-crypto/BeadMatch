...
| 文件 | 说明 |
| --- | --- |
| `walk_gen.py` | 出题算法：**引导式随机走 + 反走即解**（现在的题目全是它出的） |
...
```python
from games.beadmatch.core.free_solver import solve_any, verify, format_moves
...
```bash
python -m unittest games.beadmatch.tests.test_free_solver -v
```
