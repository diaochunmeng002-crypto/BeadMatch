...
用法：
    ...
    python -m games.beadsort.core.generator                        # 出一道题，存进题库，并打印
    python -m games.beadsort.core.generator --steps 300 --seed 7   # 指定乱走步数 / 随机种子
    python -m games.beadsort.core.generator --print-only           # 只打印，不落盘
    python -m games.beadsort.core.generator --show <题目文件>      # 读一道题并打印
"""
...
META_ORDER = (
    ...
    "balls_per_color", "level", "moves", "seed", "steps", "generator", "seconds",
    # 下面两个是采样器（core/sampler.py）出的题才有的乱度指标，可选
    "misplaced", "segments", "solution",
)
...
def generate(
    ...
    *,
    ...
    出题方式是 **walk_gen 的施工式反走**（把一条合法解倒着演一遍 → 题面）——
    不需要求解器，也不会失败；**起点一定有解**。
    """
    ...
    # 走满目标步数：施工偶尔走不下去，`walk()` 自己会换种子重试（最多 8 次）。
...
def parse_board(text: str) -> Tuple[List[List[int]], Dict[str, object]]:
    ...
    if "solution" in meta:
        ...
        # 光"能解开"还不够：每一步都得符合竞技规则（只能放空柱或同色柱顶）
        cur = freeze(matrix)
        for step, mv in enumerate(sol, 1):
            if mv not in set(legal_moves(cur)):
                raise ValueError(
                    "solution 第 %d 步 %d-%d 不合法（竞技规则：只能放空柱或同色柱顶）"
                    % (step, mv[0] + 1, mv[1] + 1))
            cur = apply_move(cur, mv)
        if not is_finished(cur):
            raise ValueError("solution 走完没把颜色收齐")
